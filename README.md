...
- `src/typing_transcription/store.cljk` — `Store` protocol +
  `MemStore`: registered batches, committed records, an append-only
  audit ledger.
- `src/typing_transcription/advisor.cljk` — `Advisor` protocol;
...
- `src/typing_transcription/governor.cljk` —
...
- `src/typing_transcription/actor.cljk` — `build-graph`,
