(ns typing-transcription.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [typing-transcription.actor :as actor]
...
(deftest commits-a-clean-low-risk-request
  (let [st (fresh-store)
        ...
        request {:batch-id "batch-1" :op :transcribe :stake :low}
...
(deftest holds-on-unregistered-batch-without-committing
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:batch-id "no-such-batch" :op :transcribe :stake :low}
