(ns typing-transcription.governor-test
...
(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-batch! st {:batch-id "batch-1" :name "Deposition Transcripts"})
    st))

(deftest ok-on-clean-transcribe
  (let [st (fresh-store)
        proposal {:op :transcribe :effect :propose :confidence 0.9 :stake :low}
...
(deftest hard-on-no-actuation-violation
  (let [st (fresh-store)
        proposal {:op :transcribe :effect :direct-write :confidence 0.9 :stake :low}
        v (governor/check {:batch-id "batch-1"} {} proposal st)]
    (is (:hard? v))
...
(deftest escalates-on-low-confidence
  (let [st (fresh-store)
        proposal {:op :transcribe :effect :propose :confidence 0.2 :stake :low}
        v (governor/check {:batch-id "batch-1"} {} proposal st)]
    (is (:escalate? v))
    (is (not (:hard? v)))))
...
(deftest store-records-and-ledger-append-only
  (let [st (fresh-store)]
    (store/commit-record! st {:batch-id "batch-1" :op :format})
