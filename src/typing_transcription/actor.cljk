(ns typing-transcription.actor
  "TypingTranscriptionActor — the ISCO-08 4131 independent typing-and-
  transcription actor as a `langgraph.graph/state-graph` (per
  ...
  The unconditional invariant: the TranscriptionAdvisor can never
  directly commit a record or dispatch a robot action the
  TypingTranscriptionGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [typing-transcription.advisor :as advisor]
            [typing-transcription.governor :as governor]
            [typing-transcription.store :as store]))

(defn build-graph
  "Build a compiled TypingTranscriptionActor graph. `store` implements
  `typing-transcription.store/Store`. `advisor` implements
  `typing-transcription.advisor/Advisor` (defaults to `mock-advisor`).
