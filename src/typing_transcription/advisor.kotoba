(ns typing-transcription.advisor
  "TranscriptionAdvisor — proposes a typing-and-transcription
  operation (transcribe a document, format an output, handle a
  confidential source document, handle a regulated source document)
  for a registered batch. The advisor is swappable: `mock-advisor`
  ...
  of batch provenance or confidential/regulated-document risk;
  `typing-transcription.governor` is the independent system that
  decides whether the proposal may proceed, per the itonami actor
  pattern.

  A proposal is a map:
    {:op :transcribe|:format|:handle-confidential-document|:handle-regulated-document
...
(def ^:private system-prompt
  "You are a typing-and-transcription advisor. Given a processing
